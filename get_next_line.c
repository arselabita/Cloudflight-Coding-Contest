...
void print_answer(int movement);
...
int main()
{
     ...
     i = 0;
     file_descriptor = open("level4/level4_1_small.in", O_RDONLY);
	...
	while (lines && lines[j])
	 {
		char **tokens1 = ft_split(lines[j], ' ');
			char **tokens = ft_split(tokens1[0], ',');
			int movementX = atoi(tokens[0]);
			int movementY = atoi(tokens[1]);
			//printf("%d\n", movement);
			print_answer(movementX);
			print_answer(movementY);
			printf("\n");
